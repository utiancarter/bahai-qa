...
from .ingest import get_all_read_online_url_exts
import os
import requests


def get_uhj_message_data_from_html(html):
    print("Parsing: " + html)
...
def save_all_uhj_messages_as_txts():
    ...
    for url_tail, summary in messages:
        ...
        date, addressee, paragraph_texts = get_uhj_message_data_from_html(url)
        with open('uhj/' + url_tail.split('/')[0] + '.txt', 'w', encoding="utf-8") as f:
            f.write(date +'\n')
            f.write(addressee +'\n')
            f.write(summary +'\n')
...
def convert_uhj_messages_txts_to_documents():
    ...
    for folder, _, files in os.walk('data/uhj'):
        for file in files:
            ...
            #TODO Add paragraph number to metadata
            metadata = {
                "author": "UHJ",
                "date": date,
                "addressee": addressee,
